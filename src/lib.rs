...
pub fn img_to_text(
	...
	let mut value_buffer:Vec<Vec<u32>>= vec![vec![0; height]; width];
	let mut counts_buffer: Vec<Vec<u32>>= vec![vec![0; height]; width];
	...
	for x in 0..img_width{
		for y in 0..img_height{
			...
			value_buffer[x_coord as usize][y_coord as usize] += luma[0] as u32;
	...
	let _standard_ascii_value_map= b"$@B%8&WM#*oahkbdpqwmZO0QLCJUYXzcvunxrjft/\\|()1{}[]?-_+~<>i!lI;:,\"^`'. ";
...
pub fn img_to_colored_terminal_text(image: RgbaImage, requested_width: Option<usize>, requested_height: Option<usize>, normalize: bool, invert_value: bool) -> String{
	...
	let mut value_buffer= vec![vec![0; height]; width];
	let mut red_buffer= vec![vec![0; height]; width];
	let mut green_buffer= vec![vec![0; height]; width];
	let mut blue_buffer= vec![vec![0; height]; width];
	let mut counts_buffer: Vec<Vec<u32>>= vec![vec![0; height]; width];
	...
	for x in 0..img_width{
		for y in 0..img_height{
			...
			value_buffer[x_coord as usize][y_coord as usize] += gray_img.get_pixel(x, y)[0] as u32;
			red_buffer[x_coord as usize][y_coord as usize] += pixel[0] as u32;
			green_buffer[x_coord as usize][y_coord as usize] += pixel[1] as u32;
			blue_buffer[x_coord as usize][y_coord as usize] += pixel[2] as u32;
			counts_buffer[x_coord as usize][y_coord as usize] += 1;
		}
	}
